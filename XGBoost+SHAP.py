...
plt.rcParams['font.serif'] = ['Arial']  #Times New Roman
...
# 加载数据
excel_path = r'C:\Users\Administrator\Desktop\me\graphene_stretch\相关系数\重要性\XGBoost+SHAP+南丁格尔玫瑰图\data_del_couple.xlsx'
data_df = pd.read_excel(excel_path)
print(f"成功从 '{excel_path}' 加载数据。")
# 目标变量
target_column_name = 'E_couple'
# 检查目标列是否存在于数据中
...
# ===================================================================
# 使用嵌套交叉验证评估模型泛化性能
# 外层交叉验证用于评估，内层交叉验证仅用于选择超参数，避免直接用调参分数
# 作为最终性能估计。若同一金属组合包含多个样本，应改用 GroupKFold。
# ===================================================================
inner_cv = KFold(n_splits=5, shuffle=True, random_state=42)
outer_cv = KFold(n_splits=5, shuffle=True, random_state=2025)
...
cv_results = cross_validate(
    estimator=nested_grid_search,
    X=X,
    y=y,
    cv=outer_cv,
    scoring=scoring,
    # 外层串行、内层并行，避免嵌套并行占用过多CPU和内存。
    n_jobs=1,
    return_train_score=False
)

# sklearn以负值表示需要最小化的误差，因此转换回正的MAE和RMSE。
...
grid_search = GridSearchCV(estimator=xgb_reg, param_grid=param_grid,
                           cv=inner_cv, scoring='neg_mean_squared_error', n_jobs=-1, verbose=2)
print("\n正在进行超参数搜索和交叉验证...")
# 使用模拟数据进行网格搜索
grid_search.fit(X, y)
# 获取搜索到的最佳模型
...
cmap = mcolors.LinearSegmentedColormap.from_list("custom_cmap", ["royalblue", "plum", "lightcoral"])  #plt.get_cmap("RdBu_r") "blue", "#4B0082", "red"
...
# 绘制外部的彩色环
ax_radial_inset.bar(x=thetas, height=[colored_ring_width] * num_vars, width=widths, bottom=inner_heights, color=colors, align='edge', edgecolor='white', linewidth=1.5)
# 遍历每个扇区，添加百分比标签
...
# 定义原始组合图的保存路径
original_image_path = r'C:\Users\Administrator\Desktop\me\graphene_stretch\相关系数\重要性\XGBoost+SHAP+南丁格尔玫瑰图\result\shap.png'
# 保存图像，设置DPI和裁剪白边
...
plt.tight_layout()
# 保存第二个图（原生蜂巢图）的路径
beeswarm_image_path = r'C:\Users\Administrator\Desktop\me\graphene_stretch\相关系数\重要性\XGBoost+SHAP+南丁格尔玫瑰图\result\shap_beeswarm_native.png'
# 保存第二个图
plt.savefig(beeswarm_image_path, dpi=208, bbox_inches='tight')
# 显示第二个图
plt.show()
# 同样，为了更好地控制图形属性，先创建一个 Figure
plt.figure(figsize=(16, 15))
...
plt.tight_layout()
# 保存
third_plot_image_path = r'C:\Users\Administrator\Desktop\me\graphene_stretch\相关系数\重要性\XGBoost+SHAP+南丁格尔玫瑰图\result\shap_beeswarm_no_labels.png'
...
ax_cbar_new = fig_combined.add_axes([cbar_left, plot_bottom, colorbar_width, plot_height]) # 在画布上添加用于颜色条的坐标轴
sm = ScalarMappable(cmap=cmap, norm=color_norm) # 创建一个可映射标量对象，用于将数据值映射到颜色
cbar = fig_combined.colorbar(sm, cax=ax_cbar_new, orientation='vertical') # 在指定的坐标轴上绘制垂直方向的颜色条
...
ax_cbar_new.text(0.5, 1.01, 'High', transform=ax_cbar_new.transAxes, ha='center', va='bottom', fontsize=30) # 在颜色条顶部添加'High'文本
ax_cbar_new.text(0.5, -0.01, 'Low', transform=ax_cbar_new.transAxes, ha='center', va='top', fontsize=30) # 在颜色条底部添加'Low'文本
cbar.outline.set_visible(False) # 隐藏颜色条的边框线
ax_cbar_new.text(-1.4, 0.5, 'Contribution for CEs ($10^4$ t)', transform=ax_cbar_new.transAxes, fontsize=30, rotation=90, va='center') # 在颜色条左侧添加旋转90度的文本标签
# --- 左图的主条形图 ---
main_ax_left = cbar_left + colorbar_width + 0.05 # 计算主条形图的左侧起始位置
ax_bar_new = fig_combined.add_axes([main_ax_left, plot_bottom, left_plot_width, plot_height]) # 在画布上添加用于主条形图的坐标轴
...
ax_bar_new.barh(y=range(len(sorted_features)), width=sorted_shap_values, color=colors, height=0.6) # 绘制水平条形图
ax_bar_new.invert_yaxis() # 反转Y轴的方向，使最重要的特征显示在顶部
ax_bar_new.set_xlabel('Contribution for CEs ($10^4$ t)', size=30, labelpad=20) # 设置X轴的标签及其字体大小和边距
...
ax_bar_new.tick_params(axis='x', which='major', direction='in', labelsize=30, length=6, pad=8) # 设置X轴主刻度的样式
...
for i, feature in enumerate(sorted_features): # 遍历每一个排序后的特征
    # 让文本右对齐到坐标轴
    ax_bar_new.text(label_x_padding, i, feature, ha='right', va='center', color='black', fontsize=30) # 在图上添加特征名称文本，并右对齐
#ax_bar_new.text(0.02, 0.98, '(a)', transform=ax_bar_new.transAxes, fontsize=30, weight='bold', ha='left', va='top') # 在主图的左上角添加子图标签'(a)'
# --- 左图的内嵌径向图 ---
...
ax_radial_inset_new = fig_combined.add_axes(inset_ax_rect, projection='polar') # 在画布上添加一个极坐标投影的内嵌图坐标轴
ax_radial_inset_new.patch.set_alpha(0) # 设置内嵌图背景为完全透明
ax_radial_inset_new.bar(x=thetas, height=inner_heights, width=widths, color=inner_colors, align='edge', edgecolor='white', linewidth=1.5) # 绘制内部的灰色/白色交替环
ax_radial_inset_new.bar(x=thetas, height=[colored_ring_width] * num_vars, width=widths, bottom=inner_heights, color=colors, align='edge', edgecolor='white', linewidth=1.5) # 绘制外部的彩色环
for i in range(num_vars): # 遍历每个特征扇区
    ...
    ax_radial_inset_new.text(label_angle_rad, label_radius, f'{percentages[i]:.1f}%', ha='center', va='center', fontsize=26) # 在扇区外添加百分比文本标签
...
ax_beeswarm = fig_combined.add_axes([right_plot_left, plot_bottom, right_plot_width, plot_height]) # 在画布上添加用于蜂巢图的坐标轴
# 使用我们最终确定的、能正确运行的函数调用
beeswarm(shap_values, max_display=len(sorted_features),ax=ax_beeswarm, show=False, color=cmap, plot_size=None) # 调用beeswarm函数绘图，并指定坐标轴、颜色表且禁用其自动尺寸调整
# 对右侧的图进行定制化修改
ax_beeswarm.set_yticklabels([]) # 移除Y轴的所有刻度标签
ax_beeswarm.set_ylabel('') # 移除Y轴的轴标题
ax_beeswarm.set_xlabel("SHAP Value (impact on model output)", fontsize=30) # 设置X轴的标签及其字体大小
ax_beeswarm.tick_params(axis='x', labelsize=30) # 设置X轴刻度标签的字体大小
# 在右上角添加标签 '(b)'
ax_beeswarm.text(0.98, 0.98, '(b)', # 在坐标轴内的相对位置(0.98, 0.98)处添加文本
                 transform=ax_beeswarm.transAxes, # 指定坐标系为坐标轴相对坐标系
                 fontsize=30, # 设置字体大小
                 fontweight='bold', # 设置字体粗细为粗体
                 va='top', # 设置垂直对齐方式为顶部对齐
                 ha='right') # 设置水平对齐方式为右对齐
# 调整右图自动生成的颜色条
if len(fig_combined.axes) > 3: # 检查是否存在由beeswarm图自动生成的颜色条（通常是第4个及以上的坐标轴）
    cbar_ax_right = fig_combined.axes[-1] # 获取最后一个坐标轴，即beeswarm图的颜色条坐标轴
    cbar_ax_right.set_ylabel('Feature Value', size=30, rotation=270, labelpad=5) # 设置颜色条的标签、字体大小、旋转角度和边距
    cbar_ax_right.tick_params(labelsize=30) # 设置颜色条刻度标签的字体大小
# =======================================================
# 保存并显示最终的组合图
# =======================================================
combined_image_path = r'C:\Users\Administrator\Desktop\me\graphene_stretch\相关系数\重要性\XGBoost+SHAP+南丁格尔玫瑰图\result\combined_shap_plot_final_style_preserved.png' # 定义最终组合图的保存路径和文件名
plt.savefig(combined_image_path, dpi=208, bbox_inches='tight') # 保存图像，设置分辨率(dpi)并裁剪掉周围多余的白边
plt.show() # 显示绘制好的图像
